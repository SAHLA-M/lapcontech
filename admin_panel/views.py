...
def admin_panel(request):
    ...
    top_brands = Brand.objects.annotate( orderitem_count=Count('products__variants__order_items') ).order_by('-orderitem_count')[:5]
    top_products = Product.objects.annotate( orderitem_count=Count('variants__order_items') ).order_by('-orderitem_count')[:10]
    top_categorys = Category.objects.annotate( orderitem_count=Count('products__variants__order_items') ).order_by('-orderitem_count')[:4]
    ...
    total_sales = int(total_sales['total'] or 0)
    total_purchase = int(total_purchase['total'] or 0)
